...
| **Background** | `linear-gradient(135deg, #1a2e40 0%, #000864 50%, #2d4a5e 100%)` with SVG illustration (stacking rectangles representing compounding value + grid pattern + connection nodes) on right side |
...
| Element | Content |
|---------|---------|
| **Badge** | "For ERP Implementers & IT Consultants" (pill badge with user-plus icon, background `rgba(0, 175, 240, 0.2)`, border `rgba(0, 175, 240, 0.3)`, text `#7EC8F2`) |
| **Headline** | `Tired of Selling Man-Hours? The Problem Isn't Your Team.` — "Man-Hours" wrapped in `<span>` with color `#00AFF0` |
...
### Benefits Box (dark: `linear-gradient(135deg, #000864, #1a2e40)` with diamond pattern overlay)
...
| Element | Content |
|---------|---------|
| **Label** | "Why Now" (JetBrains Mono, uppercase, `#00AFF0`) |
...
| **Background** | `linear-gradient(135deg, #00AFF0 0%, #2d7bc4 50%, #1e5a8a 100%)` with dot pattern overlay |
| **Padding** | 100px top/bottom |
| **Max Width** | 800px |
