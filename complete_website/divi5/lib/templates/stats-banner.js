...
@media(max-width:991px){.${p}-grid{grid-template-columns:repeat(2,1fr);gap:36px;margin:0 40px}.${p}-item:nth-child(2n)::after{display:none}}
@media(max-width:767px){.${p}-section{padding:80px 24px}.${p}-grid{grid-template-columns:repeat(2,1fr);gap:24px;margin:0}.${p}-item{padding:0 8px}.${p}-number{font-size:clamp(32px,9vw,44px)}.${p}-label{font-size:10px}}
@media(max-width:480px){.${p}-section{padding:60px 16px}.${p}-grid{grid-template-columns:1fr;gap:20px}.${p}-item::after{display:none !important}.${p}-number{font-size:clamp(36px,12vw,52px)}${data.source ? `.${p}-source{margin-top:24px;padding-top:16px}` : ''}}
/* Wave Fade */
