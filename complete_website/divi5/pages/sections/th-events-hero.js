/**
 * th-events-hero.js — Thai Events Listing hero section
 *
 * Reuses English CSS from events-listing.js heroCSS().
 * Substitutes Thai UI text (title, subtitle, back link, badges, view details).
 * Event names, dates, locations remain in English (proper nouns).
 *
 * Pattern: Direct block builder (no template — matches English events-listing.js)
 */
...
function blocks() {
  ...
  const eventCards = events.map(evt => {
    // Translate badge type if available, otherwise keep English
    const badge = thEvents.badges[evt.typeBadge] || evt.typeBadge;
    return `
    <a href="/th${evt.href}" class="${P}-card">
      <span class="${P}-badge" style="background:${evt.color}20;color:${evt.color}">${badge}</span>
      <h3 class="${P}-card-title">${evt.shortTitle}</h3>
      <div class="${P}-card-meta">
        <span class="${P}-date">${evt.date}</span>
        <span class="${P}-loc">${evt.location}</span>
      </div>
      <span class="${P}-arrow">${thEvents.ui.viewDetails}</span>
    </a>`;
  }).join('\n');
...
<section class="${P}-hero">
  <div class="${P}-inner">
    <a href="${thEvents.hero.backHref}" class="${P}-back">${thEvents.hero.backLink}</a>
    <h1 class="${P}-title">${thEvents.hero.title}</h1>
    <p class="${P}-subtitle">${thEvents.hero.subtitle}</p>
    <div class="${P}-grid">
      ${eventCards}
    </div>
  </div>
  ...
  return [
    sectionOpen({ adminLabel: thEvents.hero.adminLabel, css: `selector{padding:0!important;margin:0!important;background:#000864!important;width:100%!important;max-width:100%!important;}` }),
    ...
    codeModule(html, 'Events Listing: Thai — All upcoming events'),
    columnClose(),
    rowClose(),
    sectionClose(),
  ];
...
module.exports = {
  blocks,
  css: () => '',  // CSS is embedded in the code module's <style> tag
};
