...
| **Background** | Dark gradient: `linear-gradient(165deg, #0f1419 0%, #1a2632 40%, #000864 100%)` with grain overlay (0.03 opacity), radial glow overlays (blue + violet) |
...
| **Headline** | `Manufacturing Software by` `Manufacturing Experts` (highlight span in #00AFF0) |
...
| ERP | ERP Foundation | Blue gradient (#00AFF0 → #2d7bc4) | top: 280px, right: 60px |
...
| **Background** | Blue gradient: `linear-gradient(135deg, #00AFF0 0%, #2d7bc4 50%, #2d7bc4 100%)` with cross pattern overlay and grain texture |
| **Padding** | 140px top/bottom |
