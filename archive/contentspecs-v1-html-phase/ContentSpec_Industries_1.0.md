...
| **Background** | Linear gradient: `--navy-deep` (#0f1419) → `--navy-mid` (#1a2632) → `--navy` (#000864) at 165deg; grain texture overlay (SVG noise, opacity 0.03, 8s animation); radial gradient accent (blue glow at 70% 30% and 30% 80%) |
...
| **Headline** | `Built for Your Industry.<br><span>Proven by Experience.</span>` (h1, Noto Sans 700, clamp 40-56px, -0.03em tracking; "Proven by Experience." in `--blue` #00AFF0) |
| **Subhead** | `Your factory isn't generic — your ERP shouldn't be either. From automotive JIT scheduling to electronics component traceability to metal processing yield optimization, we've spent 44 years solving industry-specific challenges on the shop floor.` (Noto Sans, 19px, rgba white 0.75, max-width 680px; "44" uses `dw-years` dynamic class) |
...
| **Headline** | `Thailand: ASEAN's Manufacturing Hub` (Noto Sans, 40px, 700, `--navy`, -0.02em tracking) |
...
| **Headline** | `Choose Your Industry` (Noto Sans, 40px, 700, `--navy`) |
...
| **Title** | `Automotive Parts Manufacturing` (Noto Sans, 28px, #000864) |
...
| **CTA link** | `Explore Automotive Solutions →` (#00AFF0, Noto Sans 600, 15px) |
...
| Element | Content |
|---------|---------|
| **Headline** | `Cross-Industry Capabilities` (Noto Sans, 36px, #000864) |
...
| **Background** | Dark gradient: linear-gradient(135deg, #000864, #1a2e40) |
| **Padding** | 100px top and bottom |
| **Max-width** | 1000px |

| Element | Content |
|---------|---------|
| **Headline** | `Our Approach` (Noto Sans, 36px, white) |
