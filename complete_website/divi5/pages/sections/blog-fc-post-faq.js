...
function css() {
  // No section-numbered CSS — scoped to post content context.
  // Inside postContentModule, section numbers are unpredictable.
  // Use .et_pb_post_content as scope — accordion is unique within post body.
  return `
/* ═══ Blog FC Post FAQ ═══ */

/* Accordion card styling */
.et_pb_post_content .et_pb_toggle {
...
.et_pb_post_content .et_pb_toggle_title {
  padding: 16px 20px;
}
.et_pb_post_content .et_pb_toggle_content {
  padding: 0 20px 16px;
}
.et_pb_post_content .et_pb_toggle_content p {
  margin: 0;
  padding: 0;
}

/* FAQ heading — scoped by accordion proximity */
.et_pb_post_content .et_pb_accordion ~ .et_pb_text .et_pb_text_inner h2,
.et_pb_post_content .et_pb_text + .et_pb_accordion { /* structural only */ }

/* Accordion toggle icon contrast */
.et_pb_post_content .et_pb_toggle::before,
.et_pb_post_content .et_pb_toggle .et_pb_toggle_title::before { color: #666; }

/* Focus states */
.et_pb_post_content .et_pb_toggle:focus-within { outline: 2px solid #00AFF0; outline-offset: 2px; }
.et_pb_post_content .et_pb_toggle_title:focus-visible { outline: 2px solid #00AFF0; outline-offset: 2px; }

/* Responsive */
@media (max-width: 768px) {
  .et_pb_post_content .et_pb_toggle_title { min-height: 44px; display: flex; align-items: center; }
}

/* Reduced motion */
@media (prefers-reduced-motion: reduce) {
  .et_pb_post_content .et_pb_toggle { transition: none; }
}
`.trim();
}
