...
function sectionHeaderCSS(p, opts = {}) {
  ...
  const maxWidth = opts.headerMaxWidth ? `max-width:${opts.headerMaxWidth};` : '';
  const marginBottom = opts.headerMarginBottom || '64px';

  return `
.${p}-header{text-align:center;${maxWidth}margin:0 auto ${marginBottom};position:relative;z-index:2}
...
.${p}-title{font-family:'Noto Sans',sans-serif;font-size:clamp(32px,3.5vw,44px);font-weight:700;color:${titleColor};line-height:1.6;letter-spacing:-0.02em;margin:0 0 16px;padding:0}
...
.${p}-title{font-family:'Noto Sans',sans-serif;font-size:clamp(32px,3.5vw,44px);font-weight:700;line-height:1.6;letter-spacing:-0.02em;color:${titleColor};margin:0 0 16px;padding:0}
