...
| **Design Variables** | Colors, fonts, spacing | Define #3798E4, #253B50, font families once; reference everywhere |
...
|--------------|-------|-------|
| `--dw-primary-blue` | #3798E4 | CTAs, links, accents, icon backgrounds |
| `--dw-dark-navy` | #253B50 | Footer, hero overlays, dark sections |
...
| Variable Name | Value | Usage |
|--------------|-------|-------|
| `--dw-heading` | Lexend, sans-serif | All headings (H1-H6) |
| `--dw-body` | Source Sans 3, sans-serif | Body copy, descriptions |
| `--dw-mono` | JetBrains Mono, monospace | Labels, badges, stats, codes |
| `--dw-chinese` | Noto Sans TC, sans-serif | CJK fallback |
...
| **Left Panel** | Column | Background: linear-gradient(165deg, #0f1419 0%, #1a2632 40%, #253B50 100%) |
...
|---------|--------------|---------|
| **Label** | Text Module | `FOR MANUFACTURING BUSINESS OWNERS` — JetBrains Mono, 14px, uppercase, 0.15em spacing, #3798E4, with left gradient line (CSS pseudo-element or Divider module) |
| **Headline** | Text Module (H1) | `Your True Costs Are` **`Invisible.`** — Lexend 700, clamp(32px, 3.5vw, 52px), white. "Invisible" in #3798E4 (use Divi inline color) |
| **Subtitle** | Text Module | `"Shadow Excel" files have replaced your standard operating procedures. Ghost inventory means system says 100, shelf says 50. We fix this—because we've spent 44 years doing nothing but manufacturing software.` — Source Sans 3, 18px, rgba(255,255,255,0.75), line-height 1.75 |
...
Number styling: Lexend 800, 32px, #3798E4, -0.02em tracking. Label styling: JetBrains Mono, 11px, rgba(255,255,255,0.5), uppercase, 0.1em spacing.
...
Styling: Value → Lexend 700, 28px, #253B50. Label → Source Sans 3, 13px, #64748b.
...
| **Title** | `Do You Recognize These Problems?` — Lexend 700, clamp(32px, 4vw, 44px), #253B50 |
| **Subtitle** | `If any of these sound familiar, you're not alone. We've helped 50,000+ factories escape these exact traps.` — Source Sans 3, 18px, #64748b |
...
|---------|--------------|---------|
| **Number** | Text Module | Lexend 800, 64px, #3798E4 at 0.15 opacity |
| **Title** | Text Module (H3) | Lexend 700, 20px, #253B50 |
| **Quote** | Text Module | Source Sans 3, 16px italic, #475569, left border 3px solid #3798E4, padding-left 16px |
| **Description** | Text Module | Source Sans 3, 15px, #64748b, line-height 1.6 |
...
|---------|--------------------------|
| **Section Background** | linear-gradient(165deg, #0f1419 0%, #1a2632 50%, #253B50 100%) |
...
| **Product Title** | Text Module (H3) | Lexend 700, 20px, #253B50 |
| **Full Name** | Text Module | JetBrains Mono, 11px, #3798E4, uppercase, 0.05em spacing |
| **Tagline** | Text Module | Lexend 600, 15px, #253B50, line-height 1.3 |
| **Features** | Icon List Module (Divi 5 new!) | 3 items per card, blue dot bullets, each with bold term + description |
| **Benefit Line** | Text Module | 13px, #3798E4, weight 500, top border 1px solid #e2e8f0 |
...
| **Active Tab** | White background, #253B50 text, weight 600, bottom 3px blue gradient bar |
| **Inactive Tab** | Transparent, #64748b text, hover: #3798E4 text + light blue bg |
...
| Element | Divi 5 Module | Content |
|---------|--------------|---------|
| **Title** | Text Module (H3) | `Automotive Parts Manufacturing` — Lexend 700, 32px, #253B50 |
...
| Element | Divi 5 Module | Configuration |
|---------|--------------|---------------|
| **Container** | Section | Background: linear-gradient(165deg, #0f1419 0%, #1a2632 40%, #253B50 100%), padding: 100px 40px. Grain texture overlay (Code Module or background pattern). Blue radial glow overlay (background image: radial-gradient). |
...
Styling: Number → Lexend 800, clamp(48px, 6vw, 72px), #3798E4. Label → JetBrains Mono, 11px, rgba(255,255,255,0.6), uppercase, 0.12em spacing.
...
| **Each Card** | Group Module | Background: linear-gradient(135deg, #253B50 0%, #1e3a5f 50%, #0f172a 100%), radius: 20px, padding: 36px, min-height: 280px, grain overlay. Hover: translateY(-4px) + deep shadow. |
...
| **Badge** | Text Module | JetBrains Mono, 11px, #3798E4, bg rgba(55,152,228,0.15), border: 1px solid rgba(55,152,228,0.25), 5px 12px padding, 6px radius |
| **Title** | Text Module (H3) | Lexend 700, 22px, white |
| **Headline** | Text Module | Source Sans 3, 15px weight 500, rgba(255,255,255,0.9) |
| **Detail** | Text Module | Source Sans 3, 13px, rgba(255,255,255,0.55), line-height 1.65 |
...
| Element | Divi 5 Module | Configuration |
|---------|--------------|---------------|
| **Container** | Section | Background: linear-gradient(135deg, #3798E4 0%, #2d7bc4 50%, #2d7bc4 100%), padding: 120px 40px. Cross-pattern overlay (SVG background or Divi 5 pattern). White radial glow bottom-right. |
| **Content** | Call to Action Module OR Group Module | Max-width 800px, centered, z-index above overlays |
| **Title** | Text Module (H2) | `Let's Start a Conversation` — Lexend 700, clamp(36px, 4.5vw, 52px), white, -0.02em tracking |
| **Subtitle** | Text Module | `We're eager to understand your challenges and explore how we can help. No pressure, no sales pitch—just a genuine conversation about your manufacturing operations.` — Source Sans 3, 20px, rgba(255,255,255,0.9), line-height 1.65 |
...
- [ ] Fonts load: Lexend, Source Sans 3, JetBrains Mono
